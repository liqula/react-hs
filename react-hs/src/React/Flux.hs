...
-- about using persistent data structures, which is exactly what Haskell does.
--
-- In order to decide whether two @state@ or @props@ values differ, we call the haskell 'Eq'
-- instance.  So the performance of 'Eq' has some impact on performance.  If you have evidence that
-- this needs tuning in your project, we'd like to know -- please open an issue!
--
-- The predecessor of this library, <https://bitbucket/... react-flux>, did not use haskell's 'Eq'
-- to decide whether some props value changed, but javascript pointer equality with some ad-hoc
-- special cases.  This meant that two equal values were detected to be different if their evaluated
-- parts differed.  This is one of the main improvements of react-flux-hs over react-flux.
--
-- __Events__
--
...
-- the event objects delibertly do not have a 'NFData' instance, so that you must pull all your
